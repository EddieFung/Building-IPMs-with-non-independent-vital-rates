###MCEM###
logistic = function(param2, large_Y, large_X, Z_u, size) { #logistic cost given random effects
  pred = large_X %*% param2
...
likeli_con12 = function(u, part_Y1, part_pred1, part_Y2, part_pred2, sigma_g) { #likelihood of random effects
...
likeli_con3  = function(v, part_Y3, part_pred3) { #likelihood of random effects
  p  = 1/(1+exp(- part_pred3 - v))
  l3 = sum(part_Y3 * log(p) + (1-part_Y3) * log(1-p))
  return(l3)
}

rn_update = function(sig, posterior1, posterior2, noI) { #cost fn of the variance in random effects dist.
...
#cost fn of posterior mean of random effects 
...
mean_fn3  = function(v, sigma_u2, part_Y3, part_pred3) { #cost fn of posterior mean of random effects 
  p = 1/(1+exp(-part_pred3-v))
  (sum(part_Y3 - p) - v / sigma_u2**2)**2
}

sir_mean12 = function(D_inv, sigma_g, Y1, Z1, Y2, Z2, pred1, pred2, noI) { #posterior mean of random effects
...
sir_mean3 = function(sigma_u2, Y3, Z3, pred3, noI2) { #posterior mean of random effects
...
SIR_u3 = function(size, v, sigma_u2, part_Y3, part_pred3) { #sample importance resampling
  p = 1/(1+exp(- part_pred3 - v))
...
EM_SIR = function(Y1, X1, Z1, Y2, X2, Z2, Y3, X3, Z3, 
                  ...
                  noI, noI2, size, df, max_iter) { #MCEM
